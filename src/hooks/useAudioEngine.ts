...
import { saveSession, updateSession, type PreferenceSummary, buildHistoryContext } from "@/lib/sessionHistory";
...
export function useAudioEngine() {
  ...
  const [sessionSaveTrigger, setSessionSaveTrigger] = useState(0);

  const styleProfileRef = useRef<StyleProfile | undefined>(undefined);
  const abortRef = useRef<AbortController | null>(null);
  /** Current session ID for updates */
  const sessionIdRef = useRef<string | null>(null);
  /** Preference summary from history */
  const preferenceSummaryRef = useRef<PreferenceSummary | null>(null);

  const currentVersion = versions.find((v) => v.id === currentVersionId) || null;

  /** Called by SessionHistoryPanel when preferences load */
  const setPreferenceSummary = useCallback((summary: PreferenceSummary) => {
    preferenceSummaryRef.current = summary;
  }, []);
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      // Build history context from preferences
      const historyCtx = preferenceSummaryRef.current
        ? buildHistoryContext(preferenceSummaryRef.current)
        : undefined;

      const result = await callGemini(originalFile, layerOne, mode, styleTarget, undefined, undefined, historyCtx);
      if (result.error) {
  ...
  const autoFix = useCallback(async () => {
    ...
    endFix();
  }, [originalFile, originalBuffer, geminiDecision, mode, styleTarget, versions, analysis, modelUsed]);
  ...
  const sendFeedback = useCallback(async (token: FeedbackToken) => {
    ...
    const newFeedbackHistory = [...feedbackHistory, token];
    ...
    try {
      ...
      const result = await callGemini(originalFile, analysis, mode, styleTarget, token, geminiDecision, historyCtx);
      if (result.error) {
  ...
  }, [originalFile, originalBuffer, analysis, geminiDecision, mode, styleTarget, versions, feedbackHistory]);
  ...
  return {
    ...
    cancelProcessing,
    sessionSaveTrigger, setPreferenceSummary,
  };
}
